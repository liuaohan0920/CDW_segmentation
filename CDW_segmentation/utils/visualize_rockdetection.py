...
#短轴——根据垂直平分线方程的算，一共分为两个函数：

#函数一：已知俩点求垂直平分线方程
def Vertical_Med_Line(long_pair):
    ...
    A = 2 * (x_2 - x_1)
    B = 2 * (y_2 - y_1)
    C = (x_1 ** 2  - x_2 ** 2)  + (y_1 ** 2  - y_2 ** 2)
    # 同样是点（x1, y1）、（x2, y2）。那么这俩点的中垂线一般表达式Ax + By + C = 0
    # 表示如下：
    # - A = 2(x2 - x1)
    # - B = 2(y2 - y1)
...
#函数二：求出拓扑轮廓和垂直平分线的交点
def get_ShortAxis_points(long_pair,verts):
    A0, B0, C0 = get_Line(long_pair)
    A,B,C=Vertical_Med_Line(long_pair)
    print("长轴端点：")
    print(long_pair)
    minDis = 0.
    pairs=[]
    for vert in verts:
        print(vert)
        # pair.append(vert)
        print(abs(A * vert[0] + B * vert[1] + C))
        if (abs(A*vert[0]+B*vert[1]+C)<=1000):
            pairs.append(vert)
    print("候选：")
    print(pairs)
    pair0=[]
    ...
    for pair_temp0 in pairs:
        if (A0*pair_temp0[0]+B0*pair_temp0[1]+C0<=0.000000001):
            pair0=pair_temp0
        if (A0*pair_temp0[0]+B0*pair_temp0[1]+C0>=0.000000001):
            pair1=pair_temp0
    pair.append(pair0)
    pair.append(pair1)
    print(pair)
    dis = math.sqrt((pair0[0] - pair1[0]) ** 2 + (pair0[1] - pair1[1]) ** 2)
    return [pair,dis]
...
def display_instances(image, boxes, masks, class_ids, class_names,
    ...
    scale0 = image_array.shape[1] / width
    ...
    for i in range(N):
        ...
        for verts in contours:
            ...
            long_pair=pair
            A0, B0, C0 = get_Line(long_pair)
            print("垂直平分线方程为：", str(A0) + "x+" + str(B0) + "y" + str(C0) + "=0")

            #根据长轴端点计算短轴：

            A,B,C=Vertical_Med_Line(long_pair)
            print("垂直平分线方程为：",str(A)+"x+"+str(B)+"y"+str(C)+"=0")
            short_pair=[]
            short_pair,mindis=get_ShortAxis_points(long_pair,verts)
            print("短轴端点为:")
            print(short_pair[0])
            print(short_pair[1])
            print("短轴长度为：" + str(mindis) + 'pixels')
            cv2.polylines(masked_image, [np.array([short_pair], np.int)], 1,
                          (color[0] * 255, color[1] * 255, color[2] * 255), 2)
        ...
        if not captions:
            ...
            MinDis=mindis*scale
            MaxDis=MaxDis*scale
            # caption = "{} {:.3f} {:.3f}cm {:.3f}cm".format(label, score, MaxDis,MinDis) if score else label
        ...
        cv2.putText(masked_image, caption, (x1, y1 + 8), font, 1, (255, 0, 0), 2)
