...
def display_instances(image, boxes, masks, class_ids, class_names,
    ...
    for i in range(N):
        ...
        for verts in contours:
            ...
            print("长轴长度为："+str(maxdis* scale*10)+'mm')
            # print(pair)
            #若想把长轴标在输出图像上，则下面三行取消注释
            cv2.polylines(masked_image, [np.array([pair], np.int)], 1,
                          (color[0] * 255, color[1] * 255, color[2] * 255), 2)
            MaxDis = maxdis* scale*10
        ...
        if not captions:
            ...
            #若只想把label和得分显示出来，则是下一行
            #caption = "{} {:.3f} ".format(label, score) if score else label
            # 若想把混凝土的实际长度，label，得分显示在输出图像中，则下面第一行第二行取消注释）
            #MaxDis = MaxDis * scale
            #caption = "{} {:.3f} {:.3f}cm".format(label, score, MaxDis) if score else label
            # caption = "{}".format(label)
            #若想只显示实际混凝土长度，则为下面这行注释
            caption="{:.3f}mm".format( MaxDis) if score else label
        else:
            caption = captions[i]
